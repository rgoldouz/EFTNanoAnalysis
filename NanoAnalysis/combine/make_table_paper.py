...
yearLatex=['All years combined']
quark = ['DU','DC','SU','SC','BU','BC']
quarkLatex = [ '\PQu\PQd', '\PQc\PQd', '\PQu\PQs', '\PQc\PQs', '\PQu\PQb', '\PQc\PQb']
intType=['E','Mu']
intTypeLatex = ['\Pe', '\PGm']
...
for numyear, nameyear in enumerate(year):
    ...
    table += '\\begin{tabular}{|l|l|l|l|l|l|l|l|l|}' + "\n"
    table += '\\hline' + "\n"
    table += 'Vertex & $C_x$  & $C_x/\Lambda^2$   & $C_x/\Lambda^2$ & $BR_x \\times 10^{-6}$ & $BR_x \\times 10^{-6}$ '
    table += '\\\\' + "\n"
    table += '     &         &Exp. [\TeVns$^{-2}$]              &Obs. [\TeVns$^{-2}$]&Exp.               &Obs.'
    table += '\\\\' + "\n"
    table += '\\hline' + "\n"
    for numquark, namequark in enumerate(quark):
        for numintType, nameintType in enumerate(intType):
            table += '\multirow{2}{*}{$\PQt' + intTypeLatex[numintType] + quarkLatex[numquark] +'$}' 
            for numcoup, coup in enumerate(Couplings):
#                if not os.path.isdir('/hadoop/store/user/rgoldouz/FullProduction/LimitsTOPBNV/' + coup +'_T'+ namequark + nameintType+'_'+nameyear): 
