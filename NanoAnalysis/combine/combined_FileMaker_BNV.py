...
for numyear, nameyear in enumerate(year):
        ...
        for numreg, namereg in enumerate(regions):
            ...
            for f in range(len(Samples)):
                ...
                if f==0:
                    continue
                for numsys, namesys in enumerate(sys):
                    if 'jer' in namesys or 'unclusMET' in namesys or 'UnCorr' in namesys:
    ...
    #                print nameyear + namereg + ttsys[g] + str(hup.GetBinContent(b+1)) + '  ' + str(Gttsys[g][numyear][numreg][0].GetErrorYhigh(b)) + '  ' + str(Gttsys[g][numyear][numreg][0].GetErrorYlow(b))
                ...
                if 'pdf'==ttSys[g]:
                    hup.SetName(SamplesNameCombined[3] + '_' + ttSys[g] + 'Up')
                    hdown.SetName(SamplesNameCombined[3] + '_' + ttSys[g] + 'Down')
                else:
    ...
    #                print nameyear + namereg + ttsys[g] + str(hup.GetBinContent(b+1)) + '  ' + str(Gttsys[g][numyear][numreg][0].GetErrorYhigh(b)) + '  ' + str(Gttsys[g][numyear][numreg][0].GetErrorYlow(b))
                    ...
                    if SIGsys[g]=='pdf':
                        hup.SetName(SamplesNameCombined[f] + '_' + SIGsys[g] + 'Up')
                        hdown.SetName(SamplesNameCombined[f] + '_' + SIGsys[g] + 'Down')
                    else:
            ...
            for f in range(5,len(Samples)):
                ...
                hISRdown= TH1EFTtoTH1(hISRdownNormal,wc1)
                if 'TT' in SamplesNameCombined[f]:
                    hISRup.SetName(SamplesNameCombined[f] + '_tt_ISRUp')
                    hISRup.Write()
                    hISRdown.SetName(SamplesNameCombined[f] + '_tt_ISRDown')
                    hISRdown.Write()
                else:
                    hISRup.SetName(SamplesNameCombined[f] + '_Signal_ISRUp')
                    hISRup.Write()
                    hISRdown.SetName(SamplesNameCombined[f] + '_Signal_ISRDown')
                    hISRdown.Write()
